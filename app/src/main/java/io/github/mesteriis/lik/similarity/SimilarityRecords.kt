...
data class SimilarityScanStep(val examined:Int,val complete:Boolean)
...
interface SimilarityDao {
    ...
    @Transaction fun commitProgress(checkpointMediaId:String?=null,paused:Boolean=false):SimilarityProgress {
        ...
        saveCheckpoint(SimilarityCheckpoint(checkpointMediaId=checkpointMediaId,completed=value.completed,total=value.eligible,status=status,updatedAt=System.currentTimeMillis(),error=if(status==SimilarityWorkStatus.ERROR)"FINGERPRINT_FAILURE" else null))
        return value
    }
    ...
    @Transaction fun publishIfCurrent(value:ContentFingerprintRecord):Boolean {
        ...
        deleteRelations(value.mediaId);deleteScan(value.mediaId);saveFingerprint(value.copy(relationsReady=false));deleteFailures(value.mediaId);deleteBands(value.mediaId)
        if(value.perceptualVersion==PerceptualFingerprintV2.VERSION&&value.perceptualBits.size==8){saveBands(FingerprintBands.records(value.mediaId,value.perceptualVersion,value.perceptualBits));saveScan(SimilarityScanRecord(value.mediaId,value.contentRevision,value.accessEpoch,value.perceptualVersion,"",0,System.currentTimeMillis()))}
        return true
    }
...
object SimilarityRelationScanner {
    private data class Page(val examined:Int,val complete:Boolean)
    ...
    fun step(database:MediaDatabase,mediaId:String,budget:Int=SimilarityBudgets.CANDIDATES_PER_ITEM_STEP,cancelled:()->Boolean={false}):SimilarityScanStep {
        ...
        while(examined<budget){
            ...
            if(page.complete)return SimilarityScanStep(examined,true)
            if(page.examined==0)break
        }
        return SimilarityScanStep(examined,false)
    }

    private fun page(database:MediaDatabase,mediaId:String,limit:Int):Page=database.runInTransaction<Page>{
        val dao=database.similarity();val current=dao.fingerprint(mediaId)?:run{dao.deleteScan(mediaId);return@runInTransaction Page(0,true)}
        if(current.perceptualVersion!=PerceptualFingerprintV2.VERSION||dao.currentSafe(mediaId,current.contentRevision,current.accessEpoch)==null){dao.deleteScan(mediaId);return@runInTransaction Page(0,true)}
        val stored=dao.scan(mediaId)
        val scan=stored?.takeIf{it.contentRevision==current.contentRevision&&it.accessEpoch==current.accessEpoch&&it.fingerprintVersion==current.perceptualVersion}
            ?:SimilarityScanRecord(mediaId,current.contentRevision,current.accessEpoch,current.perceptualVersion,"",0,System.currentTimeMillis()).also(dao::saveScan)
        val candidates=dao.visualCandidates(mediaId,current.perceptualVersion,scan.afterMediaId,limit)
        val owners=linkedSetOf<String>()
        candidates.forEach{candidate->
            if(candidate.sha256==current.sha256)return@forEach
            val distance=runCatching{PerceptualFingerprintV2.distance(current.perceptualBits,candidate.perceptualBits)}.getOrNull()?:return@forEach
            ...
            dao.saveRelation(SimilarityRelationRecord(pair.left,pair.right,left.contentRevision,right.contentRevision,left.accessEpoch,right.accessEpoch,SimilarityKind.VISUAL,current.perceptualVersion,distance,System.currentTimeMillis()));owners+=pair.left
        }
        owners.forEach{dao.pruneRelations(it)}
        if(candidates.size<limit){dao.markRelationsReady(mediaId,current.contentRevision,current.accessEpoch,current.perceptualVersion);dao.deleteScan(mediaId);Page(candidates.size,true)}
        else{dao.saveScan(scan.copy(afterMediaId=candidates.last().mediaId,examined=scan.examined+candidates.size,updatedAt=System.currentTimeMillis()));Page(candidates.size,false)}
    }
}

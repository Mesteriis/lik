...
interface SimilarityDao {
    ...
    @Transaction fun prepareTranche(forceNew:Boolean):SimilarityCheckpoint {
        ...
        if(forceNew){return old.copy(checkpointMediaId=null,status=SimilarityWorkStatus.IDLE,updatedAt=System.currentTimeMillis(),error=null,tranche=old.tranche+1,comparisons=0,continuations=0).also(::saveCheckpoint)}
        return old
    }
...
object SimilarityRelationScanner {
    ...
    private fun page(database:MediaDatabase,mediaId:String,limit:Int):Page=database.runInTransaction<Page>{
        val dao=database.similarity();val current=dao.fingerprint(mediaId)?:run{dao.deleteScan(mediaId);return@runInTransaction Page(0,0,true)}
        ...
        val owners=linkedSetOf<String>();var comparisons=0
        ...
        if(candidates.size<limit){dao.markRelationsReady(mediaId,current.contentRevision,current.accessEpoch,current.perceptualVersion);dao.deleteScan(mediaId);Page(candidates.size,comparisons,true)}

...
class OrganizationMigrationTest {
    @Test fun versionTwoBackfillsUnicodeInBatchesAndRetriesAtomically() {
        ...
        try {
            ...
            val failed = Room.databaseBuilder(context, MediaDatabase::class.java, name).addMigrations(MediaDatabase.MIGRATION_2_3).build()
            ...
            val migrated = Room.databaseBuilder(context, MediaDatabase::class.java, name).addMigrations(MediaDatabase.MIGRATION_2_3).build()
            try {

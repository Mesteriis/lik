...
class SimilarityMigrationTest {
    @Test fun versionThirteenUpgradePreservesExactDigestsAndInvalidatesVisualReadiness(){
        ...
        try{
            ...
            val db=Room.databaseBuilder(context,MediaDatabase::class.java,name).addMigrations(MediaDatabase.MIGRATION_13_14, MediaDatabase.MIGRATION_14_15, MediaDatabase.MIGRATION_15_16).build();try{
                ...
                assertEquals(16,sql.version);assertEquals(2,count("content_fingerprint"));assertEquals(0,count("similarity_relation"));assertEquals(0,count("similarity_scan"));assertEquals(0,db.similarity().fingerprint("a")!!.relationsRevision)
    ...
    @Test fun versionTenUpgradePreservesCatalogAndCreatesEmptyRegenerableSimilarityState(){
        ...
        try{
            ...
            val db=Room.databaseBuilder(context,MediaDatabase::class.java,name).addMigrations(MediaDatabase.MIGRATION_10_11, MediaDatabase.MIGRATION_11_12, MediaDatabase.MIGRATION_12_13, MediaDatabase.MIGRATION_13_14, MediaDatabase.MIGRATION_14_15, MediaDatabase.MIGRATION_15_16).build()
    ...
    @Test fun versionElevenUpgradeInvalidatesIncompleteBandsAndEdgesAndAddsDurableScan(){
        ...
        try{
            ...
            val db=Room.databaseBuilder(context,MediaDatabase::class.java,name).addMigrations(MediaDatabase.MIGRATION_11_12, MediaDatabase.MIGRATION_12_13, MediaDatabase.MIGRATION_13_14, MediaDatabase.MIGRATION_14_15, MediaDatabase.MIGRATION_15_16).build()
            try{
                ...
                db.similarity().saveScan(SimilarityScanRecord("a",1,2,PerceptualFingerprintV2.VERSION,"",0,4));assertNotNull(db.similarity().scan("a"));assertEquals(16,sql.version)

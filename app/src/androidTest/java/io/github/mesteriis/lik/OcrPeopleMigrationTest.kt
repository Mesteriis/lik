...
class OcrPeopleMigrationTest {
    ...
    @Test fun versionEightUpgradePreservesCannotLinkProvenanceAsIndependentOwners(){
        ...
        try{
            ...
            val db=Room.databaseBuilder(context,MediaDatabase::class.java,name).addMigrations(MediaDatabase.MIGRATION_8_9, MediaDatabase.MIGRATION_9_10, MediaDatabase.MIGRATION_10_11, MediaDatabase.MIGRATION_11_12, MediaDatabase.MIGRATION_12_13, MediaDatabase.MIGRATION_13_14, MediaDatabase.MIGRATION_14_15).build()
            try{assertEquals(15,db.openHelper.writableDatabase.version);assertEquals(1,db.ocrPeople().cannotLinkOwnerCount("a","b"));assertEquals("split:person",db.ocrPeople().cannotLinkOwners("split:person").single().ownerId)}finally{db.close()}
        }finally{context.deleteDatabase(name)}
    }
    ...
    @Test fun versionNineUpgradeMaterializesNamedAndMergedAutoIdentitiesAndSurvivesRecluster(){
        ...
        try{
            ...
            val db=Room.databaseBuilder(context,MediaDatabase::class.java,name).addMigrations(MediaDatabase.MIGRATION_9_10, MediaDatabase.MIGRATION_10_11, MediaDatabase.MIGRATION_11_12, MediaDatabase.MIGRATION_12_13, MediaDatabase.MIGRATION_13_14, MediaDatabase.MIGRATION_14_15).build()
            try{
                assertEquals(15,db.openHelper.writableDatabase.version)
                db.ocrPeople().saveExposure(AiMediaExposureRecord("m",0,AiExposure.SAFE,40))

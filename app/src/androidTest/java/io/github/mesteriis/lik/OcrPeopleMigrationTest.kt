...
class OcrPeopleMigrationTest {
    ...
    @Test fun versionEightUpgradePreservesCannotLinkProvenanceAsIndependentOwners(){
        ...
        try{
            ...
            val db=Room.databaseBuilder(context,MediaDatabase::class.java,name).addMigrations(MediaDatabase.MIGRATION_8_9).build()
            try{assertEquals(9,db.openHelper.writableDatabase.version);assertEquals(1,db.ocrPeople().cannotLinkOwnerCount("a","b"));assertEquals("split:person",db.ocrPeople().cannotLinkOwners("split:person").single().ownerId)}finally{db.close()}
        }finally{context.deleteDatabase(name)}
    }
}

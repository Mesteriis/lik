...
Для Lik доступны три immutable набора: **Compact, Balanced (default), Extended**. [Task 9](models/README.md) фиксирует source/artifact SHA-256, 12 ONNX-графов и общие OCR/people/sensitive зависимости. Модели скачиваются из закреплённых Hugging Face URL только по действию в Settings и никогда не входят в APK. Task 11 готовит локальные RU/EN PP-OCRv5 и YuNet/SFace результаты в карантине; пользовательские OCR viewer/copy/search и «Люди» остаются закрыты, пока Task 13 не даст фотографии явный `SAFE` и biometric reveal контракт. Room v8 привязывает вычисленные результаты к revision/access epoch/pipeline generation и хранит имена, merge/split/move/exclusion отдельно; настройки считают только уже видимые `SAFE` строки и не раскрывают размер карантина. Физический Fold, качество на размеченном наборе, память и thermal остаются неизмеренными. См. [provenance и проверки](docs/MODEL_PROVENANCE.md), [Samsung backend policy](docs/SAMSUNG_BACKENDS.md), [AiGate](docs/AIGATE_INTEGRATION.md) и [Task 13 sensitive privacy](docs/SENSITIVE_MEDIA.md).

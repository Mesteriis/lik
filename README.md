...
Android-галерея с искусственным интеллектом. Реализованы прямой просмотр локальной медиатеки через MediaStore, ручной импорт из Google Photos через «Поделиться → Лик» и настройка иконки. ИИ пока не подключён.
...
Реализован MVP галереи с ручным импортом фотографий; все ML/AI-функции вне scope этого этапа. [План галереи](docs/superpowers/plans/2026-09-08-gallery-mvp.md) выполнен для локальной библиотеки: сетка, полноэкранный просмотр, выбор и удаление копий. Сквозная проверка фото из авторизованного облачного аккаунта Google Photos остаётся отдельной приёмкой на настроенном устройстве.
...
| --- | --- | --- |
| [Дизайн ленты и Fold](docs/superpowers/specs/2026-09-08-gallery-zoom-fold-design.md) | Мозаика дней, пять уровней масштаба времени и нижняя навигация | Реализовано собственным кодом: уровень «Фото» — три квадратные плитки в ряд, остальные уровни перестраиваются по дате съёмки; сторонний код не переносился |
| [MediaStore и Google Photos Share](docs/GOOGLE_PHOTOS_IMPORT.md) | Локальные фото читаются на месте; явно отправленные из Google Photos сохраняются как приватные копии | Собственный код `gallery/`, `imports/`, `ui/`; без OEM SDK |
| [План галереи MVP](docs/superpowers/plans/2026-09-08-gallery-mvp.md) | Сетка, полноэкранный просмотр, выбор и удаление локальных копий; ML/AI вне этапа | Реализовано собственным кодом на Android API; сторонний runtime-код не перенесён |
...
При первом запуске разрешите доступ к фотографиям. Лик показывает доступные локальные фото напрямую из системного MediaStore и не создаёт для них копии. Для облачного фото нажмите **Открыть Google Фото**, выделите фото → **Поделиться → Импорт в Лик**.

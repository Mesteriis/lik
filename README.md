...
| --- | --- |
| Реализация | Разрешённые фото MediaStore показываются без копирования; явный Share из Google Photos создаёт приватную копию. На уровне «Фото» показаны три квадратные `centerCrop`-плитки в ряд; отдельной строки «Фотографии / Фото: N» нет. |
...
| [MediaStore и Google Photos Share](docs/GOOGLE_PHOTOS_IMPORT.md) | Локальные фото читаются на месте; явно отправленные из Google Photos сохраняются как приватные копии с итогом операции | Собственный код `gallery/`, `imports/`, `ui/`; без OEM SDK |
...
Для Lik предусмотрено **несколько моделей на одном телефоне**. [Предложение архитектуры моделей](docs/MODEL_ARCHITECTURE.md) задаёт три будущих immutable versioned profiles: Compact, Balanced (default) и Extended. Каждый объединит семантический поиск, OCR и людей; модели должны собираться из внешнего проверенного cache в обычные debug/release APK. Это требование к будущей поставке: в текущем репозитории нет bundled weights, измерений или ML runtime.

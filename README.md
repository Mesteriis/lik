...
| [Дубликаты и похожие фото](docs/SIMILARITY.md) | Cross-source exact digest, отдельный perceptual fingerprint и ручное сравнение | Собственный код `similarity/`, Room v11 и WorkManager. Выдача требует current `SAFE`; только подтверждённая приватная копия переходит в существующую 30-дневную корзину. Идентичность, originals и пользовательские связи не меняются |

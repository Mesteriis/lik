...
| [Дубликаты и похожие фото](docs/SIMILARITY.md) | Cross-source exact digest groups, отдельный perceptual fingerprint и ручное сравнение | Собственный код `similarity/`, Room v13 и WorkManager. Exact members и visual candidates обрабатываются ограниченными страницами; выдача требует current `SAFE`. Только подтверждённая приватная копия переходит в существующую 30-дневную корзину |

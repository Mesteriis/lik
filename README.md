...
Для Lik доступны три immutable набора: **Compact, Balanced (default), Extended**. [Task 9](models/README.md) фиксирует source/artifact SHA-256, 12 ONNX-графов и общие OCR/people/sensitive зависимости. Модели скачиваются из закреплённых Hugging Face URL только по действию в Settings и никогда не входят в APK. Room v15 хранит локальные RU/EN OCR, people и sensitive результаты с revision/access epoch/pipeline provenance. Новые и изменённые фото скрыты до revision-matched ручного `SAFE`; верхняя кнопка временно показывает карантин после `BIOMETRIC_STRONG`, а фон, блокировка экрана, смена данных и рестарт процесса закрывают его. AiGate всё равно требует отдельного явного send. Физический Fold, калибровка sensitive-модели, качество на размеченном наборе, память и thermal остаются неизмеренными. См. [provenance и проверки](docs/MODEL_PROVENANCE.md), [Samsung backend policy](docs/SAMSUNG_BACKENDS.md), [AiGate](docs/AIGATE_INTEGRATION.md) и [sensitive privacy](docs/SENSITIVE_MEDIA.md).

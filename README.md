...
Для Lik доступны три immutable набора: **Compact, Balanced (default), Extended**. [Task 9](models/README.md) фиксирует source/artifact SHA-256, 12 ONNX-графов и общие OCR/people/sensitive зависимости. Модели скачиваются из закреплённых Hugging Face URL только по действию в Settings и никогда не входят в APK. Task 11 добавляет локальный RU/EN PP-OCRv5 и YuNet/SFace: текст показывается и копируется в viewer, участвует в обычном поиске, а экран «Люди» хранит имена и исправления отдельно от пересоздаваемых detections/embeddings. Room v7 привязывает вычисленные результаты к revision/access epoch/pipeline generation; недоступные, удалённые и не получившие явного статуса `SAFE` фотографии не попадают в OCR/people UI. Физический Fold, качество на размеченном наборе, память и thermal остаются неизмеренными. См. [provenance и проверки](docs/MODEL_PROVENANCE.md), [Samsung backend policy](docs/SAMSUNG_BACKENDS.md), [AiGate](docs/AIGATE_INTEGRATION.md) и [Task 13 sensitive privacy](docs/SENSITIVE_MEDIA.md).

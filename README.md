...
Для Lik доступны три immutable набора: **Compact, Balanced (default), Extended**. [Task 9](models/README.md) фиксирует source/artifact SHA-256, 12 ONNX-графов и общие OCR/people/sensitive зависимости. Task 10 скачивает веса и токенизаторы из закреплённых Hugging Face URL только из Settings; они никогда не входят в APK. Выбранный Balanced не считается готовым до download, hash/self-test и готовности включённых поколений. Проверенный API 37 CPU-путь выполняет image и RU query encoders всех трёх профилей; качество RU/EN, физический Fold, память и thermal остаются неизмеренными. См. [provenance и проверки](docs/MODEL_PROVENANCE.md), [Samsung backend policy](docs/SAMSUNG_BACKENDS.md), [AiGate](docs/AIGATE_INTEGRATION.md) и [Task 13 sensitive privacy](docs/SENSITIVE_MEDIA.md).

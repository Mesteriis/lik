...
Android-галерея с локальным искусственным интеллектом. Реализованы прямой просмотр разрешённой локальной медиатеки через MediaStore, ручной импорт «Google Photos → Поделиться → Лик», скачиваемые профили Compact/Balanced/Extended, семантический поиск по RU/EN-запросам и локальный разбор точных дубликатов/похожих фотографий.
...
| [Model artifacts Task 9](docs/MODEL_PROVENANCE.md) + [runtime Task 10](docs/MODEL_ARCHITECTURE.md) | Pinned OpenAI/Google/sentence-transformers/PaddlePaddle/OpenCV Zoo/Marqo weights; shared immutable profiles | Собственные `scripts/models/`, downloader/state/index/runtime в `app/.../ai/`, USearch 2.26.0 через узкий JNI и отдельный opt-in AiGate consumer. HF URL/revision/size/SHA закреплены; APK содержит только metadata/licenses и проверенные runtime-библиотеки. ORT Android 1.29.0, Gson 2.14.0 и USearch — runtime-зависимости, не модели. Sensitive-калибровка и biometric hiding остаются Task 13 |
| [Дубликаты и похожие фото](docs/SIMILARITY.md) | Cross-source exact digest groups, отдельный perceptual fingerprint и ручное сравнение | Собственный код `similarity/`, Room v14 и WorkManager. Exact members и visual candidates обрабатываются ограниченными страницами; visual readiness привязана к ревизии SAFE-домена, а PAUSED снимается только ручным запуском. Только подтверждённая приватная копия переходит в существующую 30-дневную корзину |
...
Для Lik доступны три immutable набора: **Compact, Balanced (default), Extended**. [Task 9](models/README.md) фиксирует source/artifact SHA-256, 12 ONNX-графов и общие OCR/people/sensitive зависимости. Модели скачиваются из закреплённых Hugging Face URL только по действию в Settings и никогда не входят в APK. Task 11 готовит локальные RU/EN PP-OCRv5 и YuNet/SFace результаты в карантине; пользовательские OCR viewer/copy/search и «Люди» остаются закрыты, пока Task 13 не даст фотографии явный `SAFE` и biometric reveal контракт. Room v10 привязывает вычисленные результаты к revision/access epoch/pipeline generation, материализует устойчивые пользовательские person ID и хранит независимых владельцев cannot-link решений; настройки считают только уже видимые `SAFE` строки и не раскрывают размер карантина. Физический Fold, качество на размеченном наборе, память и thermal остаются неизмеренными. См. [provenance и проверки](docs/MODEL_PROVENANCE.md), [Samsung backend policy](docs/SAMSUNG_BACKENDS.md), [AiGate](docs/AIGATE_INTEGRATION.md) и [Task 13 sensitive privacy](docs/SENSITIVE_MEDIA.md).

Референсный CLIP-путь ReFra отбрасывает кириллицу; его runtime-код не переносился. Lik использует закреплённые multilingual/OCR contracts и сохраняет Unicode NFC без удаления кириллицы.

...
The PP-OCR DB box score now follows the pinned OpenCV path: clipped floor/ceil inclusive bounds, truncate-to-int polygon vertices and inclusive `fillPoly` raster semantics. Golden vectors generated with OpenCV 4.10 cover small, slanted and clipped polygons, including a score on the 0.6 boundary. This establishes parity for the tested vectors; full publisher corpus parity and physical Fold acceptance remain open.

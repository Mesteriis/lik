...
## OCR and people · Task 11 · 9 September 2026

Room v7 stores OCR text/regions/confidence and face detections/landmarks/128-dimensional SFace embeddings by generation, media revision, access epoch and pipeline fingerprint. Empty OCR and zero-face photos receive explicit per-feature run rows, so coverage counts processed photos instead of only positive detections. A resumed job restarts its bounded keyset scan, skips current runs and therefore sees rows inserted before its previous checkpoint. Compatible mobile OCR and shared YuNet/SFace generations can be reused across profiles; server OCR remains separate. Cleanup cascades computed rows while manual person identity, naming, merge, split, move and exclusion tables remain.

The product path uses downloaded PP-OCRv5 detector/Cyrillic recognizer and YuNet/SFace graphs in the isolated, single-heavy-task CPU runtime. The viewer rejects stale OCR by request/media/revision and supports selectable/copyable RU/EN text. Conventional search accepts normalized OCR text without removing Cyrillic. The People screen exposes grouping and corrections in EN/RU Views. All OCR snippets and people rows require an available current media row plus explicit `SAFE` exposure; Task 11 never interprets uncalibrated sensitive logits as safe. JVM tests cover Unicode/CTC, detector components, publication tokens, clustering and manual resolution. API 37 Room tests cover search/view visibility, revision/access/trash transitions, reindex persistence and v6→v7 migration. Real OCR/face quality, threshold calibration, full-photo throughput, sustained thermal behavior and physical Fold acceptance remain open.

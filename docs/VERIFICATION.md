...
The fixed-size catalog snapshot retains every active and pending generation, plus only the newest complete and newest preparing generation for each accepted current or migration-compatible pipeline fingerprint. Superseded Room rows and native directories are retired after the bounded catalog becomes authoritative. A 1,200-generation regression remains within the 128-KiB payload slot while preserving active, pending and reusable compatible generations. Selecting an installed profile after a v1/v2→v3 migration can activate its verified compatible generation without reindexing. Repairing a corrupt installed artifact clears its preallocated receipt slots in place, so the repaired exact-space download does not allocate metadata after admission.
...
- `./gradlew lint testDebugUnitTest assembleDebug assembleRelease assembleDebugAndroidTest` — PASS; **114 JVM tests**, zero failures/errors/skips, lint and release shrink passed. `LIK_MODEL_CACHE=/tmp/lik-final-correction-no-model-cache ./gradlew assembleDistribution` also passed; all three APK payload gates reported zero ONNX and tokenizer payloads.
- Clean compiler-only `--no-build-cache` rebuild and `compiled_apk.py` candidate — PASS; the final candidate is byte-identical to the checked-in receipt. Reviewed policy SHA-256 `9ca9c92def7eaedc7c7007828bf3ab28079a6bf7b7ece0161051f2aad50f5987`, source fingerprint `a225134c4c77b3ab326a564a0bd6ccfa245738f982ef49a326cf4fe365e0d132`. Catalog v3 SHA-256 is `0ae75601e4ca4bcdef037dc592d25601d29a6e66e5df0d4238d2433b01c632f6`. The native-content policy SHA-256 is `610ed7cb453739112d06b9a60b371159eac32cea2ed392e36734900161d323ec`.
...
- `ANDROID_SERIAL=emulator-5580 ./gradlew connectedDebugAndroidTest` — PASS in 1m59s; **116 runner tests, zero failures/errors, 6 expected external-model/network skips**. The exact-boundary regressions perform admitted transfer, journal, ledger, catalog and receipt writes with late file allocation disabled, including repair of a corrupt artifact whose receipt entry must remain preallocated; the related rounding regression verifies that two one-byte transfers plus a one-byte safety margin require three distinct filesystem allocation units. AVD `lik_api37_qa`, API 37, arm64, 16-KiB page configuration.
...
| --- | ---: | --- | ---: | ---: | ---: |
| Debug | 60,682,791 | `f3881d8e67b3cac0965c06467f3ff2bfc134e075fed80a2f24cc74c333bfc919` | 60,621,144 | 60,928,999 | 158 |
| Release unsigned | 48,352,532 | `06ca53b9134b4d38b6b5ecd8acd3a718787459d5f9d86d04d58ad2a4e1e298d0` | 48,301,818 | 48,607,345 | 117 |
| Debug androidTest | 1,336,707 | `9b3949ea7b1e07fad9a62558f755e498632dc1b2446ef1ee6c337cce0caf8b6e` | 1,324,164 | 1,430,272 | 52 |

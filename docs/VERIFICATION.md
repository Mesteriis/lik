...
- `./gradlew lint testDebugUnitTest assembleDebug assembleRelease assembleDebugAndroidTest` — PASS; **107 JVM tests**, zero failures/errors/skips, lint and release shrink passed. `LIK_MODEL_CACHE=/tmp/lik-nonexistent-model-cache ./gradlew assembleDistribution` also passed; all three APK payload gates reported zero ONNX and tokenizer payloads.
- Clean compiler-only `--no-build-cache` rebuild and `compiled_apk.py` candidate — PASS; the final candidate is byte-identical to the checked-in receipt. Reviewed policy SHA-256 `a8234917e707e940e123dad15b83ce3f3ea189e2fbb34485506b92e321d04195`, source fingerprint `5714b01373cf932c458388bda838355a6b3d95696101e29b482a90523c1ddec2`. The native-content policy SHA-256 is `610ed7cb453739112d06b9a60b371159eac32cea2ed392e36734900161d323ec`.
- External private-model acceptance on `emulator-5580` — PASS. All three real image/Russian-text profile activations passed with the full external references hidden, proving that the mandatory production oracle works offline after model download (**3/3**, 15.958 seconds). The combined runtime/profile/index/recovery run passed **4/4** in 16.264 seconds. A separate all-component reference run covered all 12 ONNX graphs and compared complete outputs with the externally cached pinned references (**1/1**, 5.689 seconds), and a real immutable-HF ranged resume passed (**1/1**, 5.502 seconds). The 29 files / 3,663,983,902 bytes were provisioned only into emulator app-private storage and never into an APK.
- `ANDROID_SERIAL=emulator-5580 ./gradlew connectedDebugAndroidTest` — PASS in 1m58s; XML **107 tests, zero failures/errors, 6 expected external-model/network skips** (Gradle runner footer reported 113). The new exact-boundary reservation regression verifies that two one-byte transfers plus a one-byte safety margin require three distinct filesystem allocation units. AVD `lik_api37_qa`, API 37, arm64, 16-KiB page configuration.
- `LIK_MODEL_CACHE=... venv/bin/python -m unittest discover -s scripts/models/tests -p 'test_*.py'` — **46 tests, zero failures/skips**, 14.128 seconds.
...
| --- | ---: | --- | ---: | ---: | ---: |
| Debug | 60,649,579 | `1bc6a371098d2dcae6c82d92cecd21a5dcbc80f48905f7c9af8c4d0b44c4fb86` | 60,587,390 | 60,891,483 | 158 |
| Release unsigned | 48,335,856 | `f41ee8782d8778a9c4b1670af79ef9dcabb7a840d686084b3d1f989ad5709dd0` | 48,291,558 | 48,593,171 | 117 |
| Debug androidTest | 1,326,955 | `0b1e2c3f21375400343a632c3d033186ae2fe7d54bb118e07a67426fd1b67ab3` | 1,314,412 | 1,420,520 | 52 |

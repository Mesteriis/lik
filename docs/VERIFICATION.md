...
Room v8 stores OCR text/quadrilaterals/confidence and face detections/landmarks/128-dimensional SFace embeddings by generation, media revision, access epoch and pipeline fingerprint. Empty OCR and zero-face photos receive explicit per-feature run rows. A resumed job restarts its bounded keyset scan and skips current runs; a new compatible generation copies only still-current rows, then processes invalidated rows. Atomic catalog publication journals and retires superseded OCR/People generations under generation leases. Manual person identity, naming, merge, move, exclusion and split provenance remain; full split undo restores assignments and deletes exactly the cannot-links created by that split.

...
The fixed-size catalog snapshot retains every active and pending generation, plus only the newest complete and newest preparing generation for each accepted current or migration-compatible pipeline fingerprint. Search superseded IDs enter its durable retirement path around the atomic native/catalog commit; Room-backed OCR/People use the later two-phase activation contract described below. Search holds a per-generation read lease through query, Room reads and native use; cleanup takes the exclusive lease, rechecks every catalog reference while holding it, then removes Room/native state and completes only that journal entry. Startup recovery repeats the same idempotent sequence, including a crash between Room and native deletion. A 1,200-generation regression remains within the 128-KiB payload slot while preserving active, pending and reusable compatible generations. Selecting an installed profile after a v1/v2→v3 migration can activate its verified compatible generation without reindexing. Repairing a corrupt installed artifact clears its preallocated receipt slots in place, so the repaired exact-space download does not allocate metadata after admission.
...
### Task 11 review fix round 3 · 9 September 2026

OCR/People activation now writes and fsyncs a two-phase intent containing the previous and target catalog snapshots before either durable store changes. Recovery validates exact Room generation metadata and current, non-sensitive membership: it finishes the target only after the Room commit, otherwise restores the previous serving snapshot. Retirement starts only after that state is coherent. Deterministic crash-point regressions cover interruption after intent, after catalog write before Room commit, and after Room commit before intent clear; each reopen serves a complete old or new generation and retains the former serving generation.

Room v10 adds a v9→v10 data migration for historical named, merged, split or explicitly assigned `auto:*` identities. It deterministically maps them to UUID identities, preserves names and merges, materializes cluster anchors without overriding exclusions or explicit manual conflicts, and remaps split/cannot-link provenance. A real v9 database regression verifies the migrated decisions and stable identity propagation after reclustering introduces a new smaller anchor.

The PP-OCR DB box score now follows the pinned OpenCV path: clipped floor/ceil inclusive bounds, truncate-to-int polygon vertices and inclusive `fillPoly` raster semantics. The translated OpenCV 4.10.0 `CollectPolyEdges`, `FillEdgeCollection` and `LineIterator` path is limited to one convex `LINE_8` polygon. A committed offline corpus contains exact bitmasks and scores for 11 named cases plus 64 deterministic convex quadrilaterals, covering the reviewer counterexample, shallow/steep slopes, clipping, small and degenerate inputs around the 0.6 score boundary. This is strong parity evidence for the corpus, not a proof over every integer quadrilateral; full publisher corpus parity and physical Fold acceptance remain open.

Round 3 verification: the required `lint testDebugUnitTest assembleDebug assembleRelease assembleDebugAndroidTest` matrix passed; the JVM suite contains 133 tests with zero failures. Focused API 37 persistence+migration acceptance passed 13/13, including all three activation crash windows and the real v9 identity migration/recluster case. No physical device or bundled/downloaded model payload was used.

### Task 11 DB score parity · fix round 4 · 9 September 2026

The geometric point-in-polygon plus generic Bresenham approximation was replaced after a reviewer counterexample. The RED regression fixes the integer quadrilateral `(3,0),(11,3),(8,14),(0,12)` and specifically checks OpenCV pixels `(7,1)`, `(2,2)`, `(1,6)` and `(0,10)`. The GREEN implementation ports the pinned OpenCV 4.10.0 `LINE_8` line iterator and fixed-point edge scan conversion for one convex polygon. Its source URL and Apache-2.0/retained Intel notice are recorded with the code and corpus; the complete retained drawing notice is packaged in `models/licenses/OpenCV-drawing-Intel.txt`.

The committed offline corpus contains 75 exact OpenCV bitmasks and near-0.6 scores: 11 named slope/clipping/small/degenerate cases and 64 deterministic convex quadrilaterals generated with seed 41008. The focused rule test and the full 134-test JVM suite passed. The external model/packaging suite passed 46 tests with five expected platform/fixture skips after adding the pinned notice. The required `lint testDebugUnitTest assembleDebug assembleRelease assembleDebugAndroidTest` matrix passed and all APK inspections still reported zero ONNX/tokenizer payloads. API 37 was not repeated because this round changes pure Kotlin DB score rasterization and JVM metadata only; round 3's focused 13/13 persistence/migration result remains the latest device evidence. This corpus demonstrates exact parity for its cases and does not claim exhaustive proof for every possible integer quadrilateral. No physical device or `references/` content was touched.

### Task 12 exact duplicates and similar photos · 9 September 2026

Task 12 keeps media identity unchanged and adds independent exact-content SHA-256 plus versioned DCT perceptual fingerprints in Room v11. Derived ordered relations distinguish `EXACT` from `VISUAL`; bounded byte-band candidate lookup precedes Hamming verification. Automatic work requires charging and uses eight-row batches, a 64 KiB digest buffer, 64-pixel decode, persisted checkpoint, revision-scoped failure rows, and stale publication rejection. Every query and action checks current revision, access epoch, `AVAILABLE`, not trashed, and explicit `SAFE` exposure. EN/RU comparison shows source/dimensions/size and offers only confirmed private-import trash; it never mutates MediaStore/Google Photos originals or chooses a deletion automatically.

TDD evidence includes JVM RED/GREEN coverage for streaming/cancelled SHA-256, brightness/crop/dissimilar perceptual hashes, threshold boundary, band candidates, pair ordering, action capabilities, and stale tokens. Focused API 37 RED/GREEN coverage exercises cross-source exact matches independent of perceptual version, MediaStore row reuse, revision/access races, sensitive/unclassified/inaccessible/trash filtering, similar/dissimilar pairs, checkpoint resume, clipped/corrupt rows, revoked access, migration v10→v11, action routing, and stale/missing comparison UI.

The clean no-build-cache compiler matrix and the required `lint testDebugUnitTest assembleDebug assembleRelease assembleDebugAndroidTest` command passed with 141 JVM tests. The full API 37 run on `emulator-5580` finished 145 instrumentation cases with six expected external-model skips and zero failures; the target reports 16,384-byte pages. After the final bounded exact-candidate paging and last-moment UI publication guard changes, the focused T12 persistence/UI/migration run passed 13/13 on the same target. The external model/packaging suite passed 46 tests with five expected platform/fixture skips. All three APK inspections report metadata-only delivery and zero ONNX/tokenizer payloads. Reviewed compiler policy SHA-256 is `b41ee1ae1bf5e306f406dfeb217f0a6e430ad79e214f3bc47e9fe208891ee346`, source fingerprint `20080e9c7f6463cdcf62234aeacf74cfbd3fc948609e74961ae945421f2f34b7`; debug/release/androidTest APK sizes are 61,332,967 / 48,527,461 / 1,459,146 bytes. Physical Fold quality, latency, memory and perceptual-threshold calibration remain open.

...
Task 10 implements explicit Settings downloads for the three immutable Task 9 profiles. `ModelCatalog` is the sole fsynced profile state; downloads use private per-file staging, validated HTTP ranges, declared size/SHA-256, low-space reservation, durable journals, atomic publication and recovery of an already verified staging file without another request. Pause, resume, cancel, retry and inactive-profile cleanup are visible in EN/RU Settings. The old profile remains active until the selected profile has passed its real runtime self-test and enabled generations can be committed together.

The private isolated service runs stock ONNX Runtime Android 1.29.0 CPU with one heavy task at a time. API 37 acceptance executed a decoded image and a Russian query through each real profile pair: CLIP + multilingual projection (512 dimensions), SigLIP 2 Base (768) and SigLIP 2 Large (1024). Tokenizers and preprocessing came from the downloaded, hash-verified profile files. This is functional evidence only; it is not a measured quality, warm-latency, memory, battery or thermal result.

Search generations and checkpoints are stored in Room v5 with media revision, access epoch and pipeline fingerprint. Exact normalized ranking is the serving oracle. USearch 2.26.0 is built from pinned source behind a narrow JNI boundary; update/delete/search/save/load and exact/approximate parity were exercised on API 37. Background work requires user opt-in and charging/storage/battery constraints; interactive work receives the next heavy-task lease. Sensitive raw logits are scheduled and stored before ordinary indexing, but Task 13 quarantine, calibration and biometric visibility remain unimplemented and are not treated as safe on failure.

AiGate is a separate opt-in loopback client for `127.0.0.1`, with configured/discovered port, health/models/chat, bounded errors/timeouts/cancellation, no redirect broadening and no provider secret in Lik. The photo encoder decodes pixels, applies orientation, limits the long side to 1600, re-encodes JPEG below 4 MiB and removes EXIF metadata. Each request consumes a one-use send consent. The viewer boundary remains closed pending Task 13 reveal integration, and no real AiGate/cloud-provider transfer was run.

- `./gradlew lint testDebugUnitTest assembleDebug assembleRelease assembleDebugAndroidTest` — PASS; **88 JVM tests**, zero failures/errors/skips, lint and release shrink passed. All three APK payload gates reported zero ONNX and tokenizer payloads.
- Clean compiler-only `--no-build-cache` rebuild and `compiled_apk.py` candidate — PASS; byte-identical to the reviewed policy, SHA-256 `bb070e1007727af162f8a71a38d64b32427281a1b8cb1899de0f5c32c454ef86`, source fingerprint `6836c419dabd0f477adb8c3fd13569fa6432b1ae794ab4da29d5c469a0bfd41b`.
- External private-model acceptance on `emulator-5580` — **11 tests**, zero failures, 28.34 seconds. It covered all three real image/Russian-text pipelines, the Room generation path, native USearch, a real immutable-HF ranged resume, verified-staging recovery without network, Settings recreation, catalog reopen and AiGate privacy/transport paths. The 29 files / 3,663,983,902 bytes were provisioned only into the emulator app-private digest store and never into an APK.
- `ANDROID_SERIAL=emulator-5580 ./gradlew connectedDebugAndroidTest` — PASS; final XML **97 tests, zero failures/errors, 5 expected skips** for the four external-model/network cases and the Task 9 externally provisioned artifact probe. AVD `lik_api37_qa`, API 37, arm64, 16-KiB page configuration. Gradle's progress footer displayed 102; XML contains 97 test cases and is the recorded total.
...
| --- | ---: | --- | ---: | ---: | ---: |
| Debug | 60,386,747 | `ba959da38e28c309641d8e5119e8c0a3a61e165cdfd2d2eb2c8178a82d9b7f33` | 60,324,811 | 60,554,799 | 158 |
| Release unsigned | 48,259,080 | `aefa28f6ccb72cd44d62eb2ddd5da541004f34019ed16148d01755110ef5e838` | 48,213,067 | 48,438,779 | 117 |
| Debug androidTest | 1,294,514 | `f95ef5fb9f5d6dcf4c0a511b5374863eb50fa750bd010dca0c2db6d6e16cb043` | 1,282,165 | 1,367,421 | 51 |

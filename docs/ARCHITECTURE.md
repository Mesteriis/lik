...
Как в Rune Keyboard, используются системные Activity/Views и встроенный в AGP Kotlin. AndroidX Activity и LiveData/ViewModel отвечают за разрешение на фото и состояние при пересоздании экрана. PhotoLibrary держит один синхронизированный PhotoStore на процесс. Room v6 хранит метаданные, организацию, устойчивую эпоху выдачи доступа и поколения ИИ-индекса; Paging обслуживает ленту. Импорт, операции корзины, экспорт, превью и полноэкранное декодирование выполняются вне UI-потока. WorkManager очищает корзину/временный экспорт и выполняет явно включённую индексацию; отдельный foreground worker скачивает выбранные в Settings модели.

Task 10 подключает галерейный ML runtime по [архитектуре моделей](MODEL_ARCHITECTURE.md): Compact, Balanced (default) и Extended — immutable versioned sets для semantic search, OCR и people. Модели скачиваются только явным действием в Settings из immutable Hugging Face URLs; веса и токенизаторы запрещены в любом APK. Артефакты исполняются в закрытом isolated ORT-процессе, одна тяжёлая задача получает lease одновременно. USearch хранит нативное поколение поиска и возвращает ограниченный набор кандидатов для точного cosine rerank. Balanced выбран по умолчанию, но становится runtime-ready только после download, проверки, self-test и подготовки включённых поколений. Предыдущий active profile работает до атомарного переключения. OCR и People в Task 10 проходят component self-test, но их пользовательские сценарии остаются Task 11.

Manifest запрашивает `INTERNET` только для явной загрузки закреплённых моделей и отдельной opt-in loopback-интеграции AiGate; доступ к аккаунтам не запрашивается. Доступ ко всем или выбранным локальным изображениям выдаёт Android, MediaStore возвращает разрешённые `content://` URI. AndroidX добавляет собственное signature permission для внутренних receivers. Cleartext разрешён только точному loopback-хосту AiGate; резервное копирование и телеметрия отключены.
...
[Архитектура моделей](MODEL_ARCHITECTURE.md) описывает реализованные в Task 10 независимые установки, согласованные наборы функций, общий планировщик ресурсов и версионируемый индекс. Основа жизненного цикла взята из Rune Keyboard и адаптирована к нескольким моделям; веса остаются внешними скачиваемыми данными приложения.

...
- [ ] **Step 1: Write failing tests** for capture-date precedence, added-date fallback, undated ordering, day/week/month/year grouping, cross-year locale weeks, deterministic covers, and total counts using literal UTC fixtures.
- [ ] **Step 2: Run** `./gradlew testDebugUnitTest --tests io.github.mesteriis.lik.gallery.GalleryTimelineTest` and confirm compilation/failure because the model is absent.
- [ ] **Step 3: Implement** immutable timeline entries and calendar grouping with `java.time`; make every photo occur exactly once at each level and keep newest-first ordering stable by ID.
- [ ] **Step 4: Run** the focused JVM test and confirm all timeline cases pass.
...
- [ ] **Step 1: Add a failing instrumentation assertion** that a MediaStore item exposes its explicit `DATE_TAKEN` independently of `DATE_ADDED`.
- [ ] **Step 2: Run** the focused `GalleryTest` on the selected API 37 emulator and observe the missing value.
- [ ] **Step 3: Query `DATE_TAKEN`** and parse imported EXIF date/time plus offset without changing either source file.
- [ ] **Step 4: Run** focused JVM and instrumentation tests and confirm capture date controls grouping.
- [ ] **Step 5: Commit** date extraction.
...
- [ ] **Step 1: Add a failing UI test** asserting that only visible timeline cells are attached for a 120-photo library and that period rows expose their counts.
- [ ] **Step 2: Run** the focused UI test and confirm the old GridView contract fails.
- [ ] **Step 3: Add RecyclerView** through the version catalog and implement the adapter with stable IDs, span sizes, source aspect ratios in Photo mode, and a bounded bitmap cache.
- [ ] **Step 4: Run** unit tests and assemble debug to confirm the renderer compiles.
...
- [ ] **Step 1: Add failing UI tests** for default Days, direct Years selection, section placeholder/return, local-photo selection protection, and retained level after recreation.
- [ ] **Step 2: Run** the focused UI tests and observe missing controls and old local selection behavior.
- [ ] **Step 3: Replace the main layout and Activity binding**, preserve import/delete logic, show an honest placeholder for future sections, and switch timeline level by controls or one-step pinch while retaining the centered photo anchor.
- [ ] **Step 4: Run** UI tests on the selected emulator and inspect both outer-sized and inner-sized resizable windows.
...
- [ ] **Step 1: Add a failing instrumentation test** that swipes/moves to another photo, closes the viewer, and receives that photo ID.
- [ ] **Step 2: Run** the focused viewer test and confirm the result is absent.
- [ ] **Step 3: Return the current ID** on back/finish, launch the viewer for result, and simplify viewer controls over the dark photo surface while preserving delete-copy behavior.
- [ ] **Step 4: Run** viewer and gallery instrumentation tests.
- [ ] **Step 5: Commit** viewer return behavior.
...
- [ ] **Step 1: Update provenance and feature status** from selected design to implemented behavior, retaining the distinction between original code, design assets, and external references.
- [ ] **Step 2: Run** `./gradlew lint assembleDebug assembleRelease assembleDebugAndroidTest`.
- [ ] **Step 3: Run** `ANDROID_SERIAL=<api-37-emulator> ./gradlew connectedDebugAndroidTest` on the explicitly selected emulator.
...
- [ ] **Step 5: Review `git diff`** against every specification section, confirm no launcher PNG changed, and commit the final documentation/acceptance record.

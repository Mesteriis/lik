...
Transport uses bounded connection/read timeouts plus one deadline for the complete request sequence, response size limits, cancellation and stale-request identity checks. A request requires `health.running == true` and a successful `/v1/models` probe before chat. The viewer retains the active client and cancels it on opt-out, media revision, stop and destroy; disabling the feature or cancelling a photo request prevents later response publication. Loopback is verified on every redirect/reconnect and cleartext is permitted only for the exact loopback host. A photo POST is never retried automatically after an uncertain delivery outcome. Router errors and response text are untrusted display data.

...
| D05 | Несколько моделей, установка, обновление, удаление и откат | Rune + группы ReFra + assets Ente | Task 9: immutable HF download manifests, 12 ONNX graphs, no-model APK gate, strict external hash verification, host/Android evidence harness | Галерейный runtime и активация индексов — Task 10; реальные quality/Fold измерения не заявлены |
...
Уже задано пользователем: нативное Android-приложение, несколько моделей на телефоне, подход Rune к их жизненному циклу, десять выбираемых иконок и три будущих profiles — Compact, Balanced (default), Extended. Task 9 закрепил точные HF URLs/артефакты/hash; distribution gate и обычные сборки запрещают любые model payloads. Settings download и активация — Task 10. [Фактическая проверка поставки](MODEL_PROVENANCE.md) отделена от невыполненных quality/Fold измерений. Для Google Photos отдельно принят D16 — явный импорт выбранных фото, описанный ниже.
...
Pinned HF download manifests, verified external bytes, metadata-only APK enforcement and synthetic conversion evidence: [MODEL_PROVENANCE.md](MODEL_PROVENANCE.md). The optional Samsung backend remains unaccepted until [physical per-pipeline measurements](SAMSUNG_BACKENDS.md); [AiGate](AIGATE_INTEGRATION.md) is planned Task 10 work. One shared HF Marqo sensitive classifier is pinned and CPU-validated for all profiles; Settings downloads and gallery inference remain Task 10; RU/EN quality/calibration is not run. [Task 13](SENSITIVE_MEDIA.md) will implement quarantine and biometric hiding, which are not current gallery behavior.

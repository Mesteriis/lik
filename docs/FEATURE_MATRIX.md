...
| D08 | OCR для скриншотов и документов | Ente + Immich | **Реализовано в Task 11:** PP-OCRv5 mobile/server detector + Cyrillic recognizer, RU/EN display/copy, локальный поиск и точный coverage по поколениям | Реальное качество OCR и производительность Fold ещё не измерены |

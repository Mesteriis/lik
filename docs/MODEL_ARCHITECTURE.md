...
Task 11 реализует это разделение в Room v7. `ai_ocr_result`, `ai_feature_media_run` и `ai_face_detection` принадлежат generation и удаляются каскадно вместе с ним. `person_identity`, `person_face_decision`, `person_merge` и `person_cannot_link` не имеют внешнего ключа на generation: ручное намерение переживает смену профиля, reindex и очистку неактивного индекса. Результат перед публикацией повторно сверяет media ID, content revision и access-grant epoch. OCR/people запросы дополнительно требуют `AVAILABLE` и текущую запись `ai_media_exposure=SAFE`; quarantine, sensitive, inaccessible и trash не дают snippet, cover, count или навигацию. Task 13 будет владельцем классификации и биометрического reveal; Task 11 не превращает raw logits в решение `SAFE`.

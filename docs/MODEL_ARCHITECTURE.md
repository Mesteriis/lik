...
Task 11 реализует это разделение в Room v8. `ai_ocr_result`, `ai_feature_media_run` и `ai_face_detection` принадлежат generation и удаляются каскадно вместе с ним. Совместимое поколение копирует только строки с текущими revision/access epoch, а superseded поколения проходят durable journal, catalog switch, generation lease и удаление Room/native данных. `person_identity`, `person_face_decision`, `person_merge`, `person_cannot_link` и durable split provenance не имеют внешнего ключа на generation: ручное намерение и полная отмена split переживают смену профиля, reindex и очистку. Результат перед публикацией повторно сверяет media ID, content revision и access-grant epoch. Асинхронные viewer/search/people чтения повторяют эту проверку вместе с `AVAILABLE` и `ai_media_exposure=SAFE` непосредственно перед публикацией UI; quarantine, sensitive, inaccessible и trash не дают snippet, cover, count или навигацию. До Task 13 штатного пути присвоить `SAFE` нет, поэтому T11 готовит только карантинные данные и не заявляет пользовательскую доступность OCR/people.

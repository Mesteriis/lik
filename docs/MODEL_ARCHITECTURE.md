...
Настройки показывают функции **Поиск по описанию**, **Люди** и **Текст на фото**. Все три доступны независимо; включённые поколения готовятся WorkManager и активируются вместе с профилем. В деталях профиля видны модели, версия, место на диске, загрузка/ошибка, прогресс индексации и доступные действия. Устанавливаются только три проверенных preset-набора из закреплённых Hugging Face URLs. Произвольные URL, импорт файлов моделей и пользовательские профили не поддерживаются.
...
Task 11 реализует это разделение, Task 13 добавляет privacy-таблицы, а final review доводит схему до Room v16. `ai_ocr_result`, `ai_feature_media_run` и `ai_face_detection` принадлежат generation и удаляются каскадно вместе с ним. Совместимое поколение копирует только строки с текущими revision/access epoch и явным `SAFE`; people clustering выполняется вне монитора каталога и write-транзакции, отменяется между сравнениями и обрабатывает страницы до 128 лиц. Complete-link внутри каждой страницы сохраняет консервативную границу совпадений; один человек может получить несколько автоматических групп между страницами, которые пользователь может объединить. Короткие транзакции сохраняют страницы, а финальная проверка membership/catalog revision, пересчёт totals и catalog switch выполняются атомарно. Все поколения, реально вытесненные глобальным catalog bound, проходят durable journal, generation lease и повторную проверку ссылок перед удалением Room/native данных. `person_identity`, `person_face_decision`, `person_merge`, `person_cannot_link`, many-owner cannot-link provenance и durable split provenance не имеют внешнего ключа на generation: именование материализует устойчивую привязку группы, v9→v10 переводит старые именованные/merged `auto:*` идентификаторы в детерминированные UUID с durable anchor assignments, а отмена split снимает только вклад этого split. Асинхронные viewer/search/people/coverage callbacks непосредственно перед изменением Views повторно сверяют generation/request, media ID, content revision, access-grant epoch, `AVAILABLE`, отсутствие trash и `ai_media_exposure=SAFE`; Room invalidation очищает уже показанные устаревшие данные. Room v15 добавляет provenance-bound automatic/run записи и отдельный manual override; только их разрешённый итог материализуется в `ai_media_exposure`.

Room v16 хранит отдельный trigger-maintained `catalog_change_state`: он учитывает и скрытые/trash строки, не меняя SAFE-only revision/budgets Similarity. Burst-события каталога/ручного SAFE объединяются, WorkManager добавляет screening → включённые Search/OCR/People к текущей цепочке; opt-out исключает соответствующий индекс. Все multi-pipeline операции берут один sorted/deduplicated набор удерживаемых fingerprint locks. Финальный organization render сверяет этот revision и reveal epoch, а явные restore/purge заново проверяют актуальные row/decision/lease в транзакции. Автоматический retention purge остаётся отдельным unauthenticated maintenance.

People paging использует составной индекс `(generationId,detectionId)` и обязательную нижнюю границу ключа; обновление кластеров использует `(generationId,anchorId)`. Проверка реального SQL, перехваченного у Room DAO, требует range seek без временной сортировки и индексированное обновление anchors: 128-face страницы не превращаются в повторный полный scan поколения.

Большой probability-map PP-OCR не проходит через Binder `Bundle`: вызывающий процесс выделяет ограниченный `SharedMemory`, isolated runtime пишет туда результат и возвращает только размер и SHA-256. Image/text/float/smoke inference используют зарегистрированные cancellable RunOptions; отменённые queued requests удаляются до открытия графа, late cancellation не сохраняет ID. Interactive admission прерывает background inference, а shared classifier использует тот же priority/thermal gate. Timeout/interruption посылает request-scoped ORT cancellation, а невозможность отправить ответ завершает isolated process и инвалидирует lease. DB postprocess использует threshold 0.3, box score 0.6, максимум 1000 контуров, quadrilateral/unclip 1.5 и перспективную rectification. CTC recognizer уже выдаёт Softmax posterior: confidence берётся непосредственно из конечных вероятностей, с проверкой finite/range/sum; повторного softmax нет. SFace получает least-squares similarity alignment по всем пяти YuNet landmarks в 112×112.
...
Снимок `ModelCatalog` имеет фиксированный двухслотовый размер. Он всегда сохраняет active/pending поколения и по одному последнему complete и preparing поколению для каждого допустимого current/compatible fingerprint; остальные поколения удаляются из снимка до записи, затем их Room/native данные выводятся из эксплуатации. Поиск удерживает read lease поколения на всём пути Room/native use, а cleanup получает exclusive lease, повторно проверяет указатели каталога и выполняется после завершения читателей. Для Room-backed OCR/People двухфазный fsynced activation intent хранит предыдущий и целевой снимки до изменения Room/catalog. Startup recovery проверяет полную Room membership и завершает целевое состояние либо восстанавливает прежний serving snapshot; retirement journal создаётся только после согласованного commit/recovery, поэтому serving generation не удаляется в crash window. Для native Search действует существующий atomic publication path; startup recovery идемпотентно завершает разрешённое удаление. Удаление неактивного профиля одной synchronized catalog-операцией повторно проверяет состояние профиля и все глобальные active/pending ссылки; в отдельный reasoned journal попадают только фактически removable IDs, поэтому shared OCR/People generation другого профиля не очищается при recovery. Совместимый индекс установленного профиля после миграции v1/v2→v3 активируется без повторной индексации. Repair повреждённого artifact очищает уже выделенный receipt slot на месте и не создаёт новый metadata-файл после проверки свободного места.
...
Все профили дополнительно используют один shared `sensitive-v1` (Marqo ViT-Tiny), без дублирования байтов и без фиктивного порога. Калибровка на реальных данных не выполнена, поэтому raw output не публикует автоматический `SAFE`. [Task 13](SENSITIVE_MEDIA.md) реализует quarantine новых/изменённых фото, отдельные ручные overrides и временный `BIOMETRIC_STRONG` reveal. [AiGate](AIGATE_INTEGRATION.md) сохраняет отдельный opt-in и одноразовое разрешение отправки для каждого фото.

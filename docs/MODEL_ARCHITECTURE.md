...
External QA self-test metadata retains the pinned smoke-reference filename, size/SHA-256, slice and tolerance for every OCR, face and sensitive graph. The reference bytes remain outside Git/APK; when provisioned into the QA cache, the runtime compares actual outputs against them instead of comparing a run to itself. Product installs still run representative nonzero semantic/name/shape checks after verified downloads.

...
The Android probe takes explicitly provisioned development files in the debug emulator's private `files/model-probe`, checks every model and reference hash, runs all 12 graphs on ORT CPU, compares complete outputs with host references, and samples PSS every 100 ms. APKs still contain zero models. Run `verify_hf_runtime.py` to create the hash-checked synthetic references in `hf-runtime-candidates/`, and install debug/test APKs onto an explicitly selected emulator before provisioning. `freeze_hf_catalog.py` deterministically extracts the catalog v2 activation samples from those hash-verified references; the catalog stores only non-model float samples, expected output shape/hash binding and tolerances. Product activation does not require the full `.f32` files: it executes every downloaded graph and compares it against the mandatory catalog samples, so subsequent self-tests work offline. No physical device is selected automatically.

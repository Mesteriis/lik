...
The catalog state codec has a fixed two-slot capacity. It retains active and pending generations and only the newest complete/preparing generation for each accepted current or compatible pipeline fingerprint. Searches hold a generation read lease through Room/native use; retirement is journaled before the catalog switch, waits for readers, rechecks catalog authority and resumes idempotently after a crash. Inactive-profile removal atomically recomputes globally referenced generations, and its reasoned journal cannot clear a shared generation activated by another profile. Artifact repair preserves the preallocated receipt entry, so a repaired transfer does not create metadata after its exact-space admission.

...
- The offline profiles are Compact, Balanced (the default), and Extended. They are immutable versioned search/OCR/people sets; user-supplied profiles are unsupported. Every APK is metadata-only: weights and tokenizers are explicitly downloaded from pinned Hugging Face URLs in Settings and must never be bundled. External developer caches support verification only; model quality/calibration and physical Fold measurements remain separate acceptance gates.
